...
export default function MidiaKitRegistroPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!form.name || !form.email || !form.instagram || !form.password) {
        ...
        try {
            const existing = await mockDb.midiaKitUser.findByEmail(form.email)
            if (existing) {
            ...
            await mockDb.midiaKitUser.create({ data: form })

            // Store session in localStorage for mock auth
            localStorage.setItem("midiakit-user", JSON.stringify({ email: form.email, name: form.name, instagram: form.instagram }))
                    ...
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <p className="text-center text-sm text-muted-foreground">
                                Já tem conta?{" "}
                                <Link href="/login/midia-kit" className="text-emerald-500 hover:underline font-medium">
                                    Fazer login
                                </Link>
                            </p>
                        </form>
